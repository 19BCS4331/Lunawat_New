...
import { useLoginWithPasswordForm } from '@/validation';
import { colors, spacing } from '@/theme';
import type { LoginWithPasswordForm } from '@/validation';
...
export default function PasswordLoginScreen() {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitting }, setError, reset } = useLoginWithPasswordForm();
  const loginWithPasswordMutation = useLoginWithPassword();
  const setAuth = useAuthStore((state) => state.setAuth);
  const { alertState } = useCustomAlert();
  ...
  const onSubmit = async (data: LoginWithPasswordForm) => {
    ...
    try {
      ...
      if (errorMessage === 'Invalid User' || errorMessage.includes('Invalid')) {
        ...
        setError(field, { message: t('passwordLogin.loginFailed') });
      }
    }
  };
