...
import { useOnlinePaymentHistory } from '@/hooks';
import type { OnlinePayment } from '@/types';

export default function OnlinePaymentHistoryScreen() {
  const router = useRouter();
  const { data: payments, isLoading, error } = useOnlinePaymentHistory();
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.white }}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: spacing[6] }}>
          <EmptyState
            ...
            description="Failed to load online payment history. Please try again."
            actionLabel="Retry"
            onAction={() => router.replace('/payments/online-history')}
          />
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.white }}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, padding: spacing[6] }}>
        ...
        {payments && payments.length > 0 ? (
          payments.map((payment: OnlinePayment) => (
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: spacing[2] }}>
                ...
                <View
                  style={{
                    ...
                    borderRadius: 4,
                    backgroundColor: payment.Status === 'Success' ? colors.success[100] : colors.error[100],
                  }}
                >
                  <Text
                    style={{
          ...
          <EmptyState
            ...
            description="You don't have any online payment transactions."
          />
        )}
      </ScrollView>
    </SafeAreaView>
